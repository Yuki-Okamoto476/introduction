...
- MVP リリース → 顧客からフィードバック（FB）回収 → 改善、のサイクルを速く回すために、開発側として PdM やデザイナーと調整して開発工数が大きくビジネスインパクトが小さそうな部分はスコープを削って高速で開発（フロントエンドからバックエンドまで一人称で開発して組織図の表示は 1 週間、組織図の編集（初期 ver）は 1 ヶ月でリリース）。
- 組織図の表示・組織図の編集（初期 ver）リリース後は自分が顧客との打ち合わせを主導。具体的には、初期 ver の機能の説明、次 ver の要件の調整を行なった。以下工夫した点。
  ...
  - 次 ver で必要そうな要件について、顧客の反応を見ながら実際に導入する上でマストの要件とベターの要件を判断。要件の叩き台を作ったのち、デザイナーと開発者から意見を回収し要件定義。
