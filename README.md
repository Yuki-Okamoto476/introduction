...
- 開発者やデザイナー向けのより詳細な仕様はドメインモデル図を作成しその中に記載。これを元に開発者やデザイナーとドメインモデリングを行い、仕様に関しての議論・画面設計・DB 設計・クラス設計をメンバーと進めた。
...
- クリティカルパスを考慮しながらタスクをアサインし毎日の朝会で進捗を確認。進捗が遅れている場合は各タスクのスケジュール or スコープを調整したり、メンバーが体調不良で数日休んでいた場合はそのメンバーのタスクを一部巻き取ることでスケジュールの遅延を防いだ。
- バックエンド、フロントエンドともに自分が設計やコードのレビューを担当することで、仕様やクオリティの担保を行なった。
- 開発と並行して QA チームにテスト設計を依頼。QA チームの工数とスケジュールを鑑み、テスト設計をもとに開発チームメンバー全員で打鍵テストを行うことを決定。
