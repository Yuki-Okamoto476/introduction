...
#### 担当業務
サービス共通の基盤チームの開発リーダー。（2023.11〜）<br>
チームのマネジメント、プロジェクトの管理、要件定義、設計、開発、テスト、コードレビューを行なっている。

#### チーム構成
リーダー1人（自分）、開発者4人

#### 使用技術
React | TypeScript | NestJS | GraphQL | Terraform | GCP
...
##### 【行動】
以下を行なった。
- 競合プロダクトの調査から入り要件定義。他チームのプロダクトマネージャーとディスカッションしながら仕様を策定。
- 画面設計を行い、デザイナーとディスカッションしながら画面作成。
- 仕様から必要なDB設計、DB作成。
- 仕様から必要なタスクを分解しメンバーにアサイン。
- アサインした開発のコードレビューや進捗管理。
- ビジネス側や他チームとの調整。

#### 【結果】
開発期間1ヶ月という目標通り開発を終わらせることができた。
...
具体的には、Playwrightで全画面のスクリーンショットを撮影、スクリーンショットをGCS（Cloud Storage）に保存、最新のブランチのスクリーンショットと比較し差分があれば検知してくれる仕組みをGithub Actions（CI）で構築。
...
- 整理した事項に基づき、以下を全て一人称で担当
  - インフラ：SREチームのメンバーと連携をとりながら、移行にあたって必要なTerraformの修正やGCPの設定を追加
  - BE：新規のマイクロサービスへの移行、APIの修正・新規作成
  - ︎FE：修正したAPIの繋ぎ込み部分を修正

#### 発揮したバリュー6
##### 【課題・状況】
Sassを契約したクライアントにアカウントを発行する際に、契約日当日に担当者が発行する必要があった（ex. クライアントの使用開始日が2023/10/1の9:00だった場合、その日時ちょうどに担当者がアカウントを発行する）。

##### 【行動】
アカウントの発行忘れやその日時ちょうどに待機しておかなければならない不便さから、あらかじめ日時を入力しておけばその日時に自動で発行される予約機能を作成。<br>
具体的には、GCPのworkflowsから予約実行できるようにTerraformを用いてworkflowsを作成し、もし予約した日時にアカウントの作成が失敗した場合はslackで通知がいくようにした。
