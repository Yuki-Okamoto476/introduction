...
要求定義・要件定義・開発マネジメントに加え、営業活動まで一貫して担当。

- 競合優位性を作るために社内の人事部門と連携。性格検査結果に基づいて上司・部下の相性を数値化する独自のマッチングアルゴリズムを開発。新規性が高かったため、発明として特許出願に至った。
- バックエンド 1 名、フロントエンド 1 名、デザイナー 1 名という少人数構成の中、2 ヶ月での開発完了を目標に QCD 管理を徹底。スコープ・スケジュール・品質・リスクマネジメントを担い、安定した進行を実現。
- 営業リスト（従業員 3,000 名以上のエンプラ企業）に対して 170 件のテレアポを自ら実施し、4 件の商談獲得に成功。また、自身が獲得した商談は自らプレゼン・提案を担当。他営業担当が獲得した案件にも開発責任者として同席し、技術的・企画的な観点から価値提案を行った。
- 自ら商談していく中で、上司と部下の相性という判断軸だけでは配属効率化というソリューションを訴求するのは難しいと判断。そこで、営業チームに対して「配属効率化というニーズはありそうだが、上司と部下の相性という文脈だと新卒の離職率改善という訴求の方が刺さるのではないか」と自ら提言し、営業の訴求の方針をピボットさせた。
- 企画前のユーザーインタビュー（自身もインタビュワーとして参加）で大企業の人事部は配属案検討の工数がかなりかかっている事実があること、自分が商談を行った企業も同様の課題を抱えていること、競合にも同じような機能があることから、配属効率化のニーズは確実にあると考え引き続き足りない機能を増やしていくことにした。配属を決めるために使うデータは各社様々でどれから優先的に開発するか迷ったが、10 社ほどに行ったインタビューや商談を振り返り、業界問わず「配属先部署の必要人数」を加味した配属は基本的に行っていることや開発工数の観点からまずは必要人数を加味した配属案出力の機能を作るという企画書を作成し提案。

##### 【結果】

本機能に価値を感じてくれたとある 1 社のエンプラ企業への導入が決定。引き続き営業活動・機能の拡充に取組中。
